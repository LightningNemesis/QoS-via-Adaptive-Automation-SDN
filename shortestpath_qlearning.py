# -*- coding: utf-8 -*-
"""ShortestPath_QLearning.ipynb

Automatically generated by Colaboratory.

Original file is located at
    https://colab.research.google.com/drive/1bwtJIF8KvTOUwE98uf-y3spo6R62GFhx
"""
...
edge = [(2,3), (3,2), (2,1), (1,2), (1,5), (5,1), (5,4), (4,5), (4,3), (3,4)] # *DYNAMIC
edges = []
for e in edge:
...
for key in G[4]: # *DYNAMIC
  # print(key)
  R[key,4] = 100 # NEED TO CALCULATE REWARD FOR EVERY LINK DYNAMICALLY, VIA INPUTS FROM PYGNMI
print(R)
...
'''
def nextNode(start, exp_rate): # exp_rate -> exploration rate (higher exp_rate = more probability of choosing a random node)
  random_value = random.uniform(0,1)
  
  if random_value < exp_rate: # if random_value is lower than exp_rate, choose the next node randomly
    sample = G[start]
  else:
    sample = np.where(Q[start,] == np.max(Q[start,]))[1] # finds index of the highest Q-Value in "start"'s row
...
def updateQ(node1, node2, alpha, gamma): # alpha: learning rate, gamma: discount factor
  max_index = np.where(Q[node2,]== np.max(Q[node2,]))[1] # finds index of the highest Q-Value in that row
  if max_index.shape[0] > 1:
    max_index = int(np.random.choice(max_index, size = 1))
  else:
    max_index = int(max_index)
  max_value = Q[node2, max_index]
  Q[node1, node2] = int(Q[node1, node2] + alpha*(R[node1,node2] + gamma * max_value - Q[node1, node2]))
